var playerAPF = false
var playerAP = undefined
var playerAlastDic = undefined;
...
window.playerA = new (class PlayerControl {
  ...
  land(aMyTankCount, aTankCount, aBulletCount, aMyBulletCount1, aMyBulletCount2, aworld, screenX, screenY, config, PF) {
    ...
    for (const enemy of enemyTanks) {
      ...
      if (misDistanceOfEnemy < 6 * currentTankWH && (undefined != playerAP && playerAP.length < 8)) {
        playerAPF = false
      }
    }
  ...
  */
        ...
        if (undefined != lateEnemy) {
          ...
          //追击 2倍自身距离是最小追击单位，小于这个距离，不能躲开正面攻击
            ...
            if (playerAPF == true) {
              ...
              if (undefined == testpath || undefined == playerAP || testpath.length <= playerAP.length && testpath.length < 10) { 
                playerAPF = false 
              }
              ...
              if(playerAPF == true)
              {
                ...
                var PFDOWN = playerAP[0][0] == playerAP[1][0] && playerAP[0][1] < playerAP[1][1]
                var PFUP = playerAP[0][0] == playerAP[1][0] && playerAP[0][1] > playerAP[1][1]
                var PFRIGHT = playerAP[0][0] < playerAP[1][0] && playerAP[0][1] == playerAP[1][1]
                var PFLEFT = playerAP[0][0] > playerAP[1][0] && playerAP[0][1] == playerAP[1][1]
                ...
                } else if (true == PFLEFT && false == PFTLEFT && PFCtestY >= playerAP[0][1] && PFFtestY == playerAP[0][1]) {
                  testmoveDirection = this.#DIRECTION.UP
                }
                else if (true == PFLEFT && false == PFTLEFT && PFCtestY == playerAP[0][1] && PFFtestY <= playerAP[0][1]) {
                  testmoveDirection = this.#DIRECTION.DOWN
                }
                else if (true == PFRIGHT && false == PFTRIGHT && PFCtestY >= playerAP[0][1] && PFFtestY == playerAP[0][1]) {
                  testmoveDirection = this.#DIRECTION.UP
                } else if (true == PFRIGHT && false == PFTRIGHT && PFCtestY == playerAP[0][1] && PFFtestY <= playerAP[0][1]) {
                  testmoveDirection = this.#DIRECTION.DOWN
                } else if (true == PFUP && false == PFTUP && PFCtestX == playerAP[0][0] && PFFtestX <= playerAP[0][0]) {
                  testmoveDirection = this.#DIRECTION.RIGHT
                }
                else if (true == PFUP && false == PFTUP && PFCtestX >= playerAP[0][0] && PFFtestX == playerAP[0][0]) {
                  testmoveDirection = this.#DIRECTION.LEFT
                }
                else if (true == PFDOWN && false == PFTDOWN && PFCtestX == playerAP[0][0] && PFFtestX <= playerAP[0][0]) {
                  testmoveDirection = this.#DIRECTION.RIGHT
                } else if (true == PFDOWN && false == PFTDOWN && PFCtestX >= playerAP[0][0] && PFFtestX == playerAP[0][0]) {
                  testmoveDirection = this.#DIRECTION.LEFT
                } else {
                  testmoveDirection = this.#DIRECTION.BLOCK
                  console.log("%c%s", "color: red", "到达路径", playerAP[0][0], playerAP[0][1])
                }
            ...
            //没有寻路
            if (false == playerAPF && testpath.length<15) {
              if ((disX < disY) && (lateEnemy.Y < currentTankY)) {
                if (true == testRIGHT && lateEnemy.X > currentTankX) {
                  moveDirection = this.#DIRECTION.RIGHT;
                } else if (true == testLEFT && lateEnemy.X < currentTankX) {
              ...
              if ((disX < disY) && (lateEnemy.Y > currentTankY) && false == playerAPF) {
              ...
              if ((disX > disY) && (lateEnemy.X > currentTankX) && false == playerAPF) {
            ...
            if (undefined == moveDirection) {
              ...
              console.log(false == playerAPF?"格斗":"寻路", moveDirection)// 这里应该发动A星
            }
          }
  ...
  #TestFireDis(disX, disY, dis, currentTankDirect, testbaseUP, testbaseDOWN, testbaseRIGHT, testbaseLEFT, currentTankWH, lateEnemy, currentTank,testpath) {
    ...
    if (/*(disX > firedirectdis * currentTankWH || disY > firedirectdis * currentTankWH) ||*/ dis >= currentTankWH * 2 && testpath.length<9) {//调整炮口
      ...
      if (undefined != moveDirection && playerAlastDic !=moveDirection) {
        playerAPF = false
